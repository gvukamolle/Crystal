...
- **Встроенный онбординг** — автоматическая установка зависимостей с интерактивными инструкциями для macOS/Windows/Linux
...
- **Debug mode Easter egg** — скрытый режим отладки для тестирования функционала
...
│                                                                  │
│  ┌──────────────────────────────────────────────────────────┐    │
│  │              Onboarding System (settings.ts)             │    │
│  │  - Автоопределение Node.js и Claude CLI                 │    │
│  │  - NodeInstallModal — инструкции для macOS/Win/Linux    │    │
│  │  - IntegrationGuideModal — OAuth авторизация            │    │
│  │  - Platform-specific terminal commands (с кнопками)     │    │
│  │  - DebugModeConfirmModal — Easter egg активация        │    │
│  └──────────────────────────────────────────────────────────┘    │
└──────┬───────────────────────────────────────────────────────────┘
       │
  ┌────▼──────┐
...
Интерактивная система помощи с установкой зависимостей и первоначальной настройкой плагина.

### Архитектура

**Компоненты:**
- **Автоопределение зависимостей** — проверка Node.js и Claude CLI при запуске
- **NodeInstallModal** — модалки с инструкциями для macOS/Windows/Linux
- **IntegrationGuideModal** — пошаговая OAuth авторизация
- **Platform-specific commands** — терминальные команды с кнопками запуска
- **DebugModeConfirmModal** — Easter egg для активации режима отладки

### Процесс онбординга

**1. Проверка зависимостей (при открытии ChatView):**
```typescript
// Проверка Node.js
const nodeVersion = await checkNodeVersion();
if (!nodeVersion) {
  // Показать NodeInstallModal с инструкциями для платформы
}

// Проверка Claude CLI
const cliStatus = await checkCLIInstalled(agent.cliPath);
if (!cliStatus.installed) {
  // Показать инструкции по установке CLI
}
```

**2. Модалки установки Node.js:**

| Платформа | Инструкции |
|-----------|------------|
| **macOS** | Homebrew (рекомендуется) + официальный установщик |
| **Windows** | Официальный установщик + Chocolatey + winget |
| **Linux** | apt/dnf/pacman + NVM (рекомендуется) |

Каждая инструкция включает:
- Описание шагов
- Блоки с командами терминала
- Фиолетовые кнопки-стрелки для запуска команд в системном терминале
- Ссылки на официальную документацию

**3. Интерактивные команды терминала:**

Для каждой команды установки:
```typescript
const cmdContainer = createDiv({ cls: "crystal-install-command-container" });
cmdContainer.createEl("code", { text: command });

const runBtn = cmdContainer.createEl("button", {
  cls: "crystal-run-command-btn",
  attr: { "aria-label": "Выполнить в терминале" }
});

runBtn.addEventListener("click", () => {
  this.plugin.openInSystemTerminal(command);
});
```
...
- `curl ... | sudo -E bash - && sudo apt-get install nodejs` — Ubuntu/Debian
- `sudo dnf install nodejs` — Fedora
- `sudo pacman -S nodejs npm` — Arch
- `nvm install --lts` — NVM (все платформы)

**4. OAuth Guide (IntegrationGuideModal):**

Пошаговые инструкции:
1. Установка CLI: `npm i -g @anthropic-ai/claude-code`
2. Запуск авторизации: `claude`
3. Вход через браузер (OAuth)
4. Проверка статуса: кнопка "Начать интеграцию"

**5. Debug Mode Easter Egg:**

Скрытая функция активации debug mode:
- Клик на версию CLI в настройках (подчёркивается при наведении)
- Модалка подтверждения с двумя кнопками:
  - "DON'T DO IT, THERE'S NOTHING INTERESTING" (красная, отмена)
  - "Да, мне надо" (фиолетовая, активация)
- При активации показывает секцию Debug/Testing с симуляцией отсутствия Node.js

### Локализация

Все модалки и инструкции локализованы на 8 языков:
- Русский, English, Français, Deutsch, Español, हिन्दी, 中文, 日本語

**Новые ключи локализации (settingsLocales.ts):**
- `nodeNotFound`, `nodeInstallTitle`, `nodeInstallDesc`
- `nodeInstallStep1`, `nodeInstallStep2` (macOS)
- `nodeWinInstallerDesc`, `nodeWinChocolatey`, `nodeWinWinget` (Windows)
- `nodeLinuxUbuntu`, `nodeLinuxFedora`, `nodeLinuxArch`, `nodeLinuxNVM` (Linux)
- `installViaTerminal` — tooltip для кнопок запуска команд

### Стили

**CSS компоненты (styles.css):**
```css
.crystal-install-command-container {
  display: flex;
  align-items: center;
  gap: 8px;  /* Команда + кнопка */
}

.crystal-run-command-btn {
  width: 32px;
  height: 32px;
  background: var(--interactive-accent);  /* Фиолетовый */
  border-radius: 6px;
}

.crystal-run-command-btn:hover {
  transform: translateX(2px);  /* Анимация движения вправо */
}

.crystal-debug-trigger {
  cursor: pointer;
...
1. **Zero configuration** — пользователь не открывает документацию, всё в UI
2. **Platform-aware** — автоопределение ОС и платформо-специфичные инструкции
3. **One-click execution** — команды запускаются в терминале по клику
4. **Локализация** — поддержка 8 языков из коробки
5. **Guided flow** — пошаговый процесс без пропущенных шагов
...
**Общая статистика:**
- **TypeScript код:** ~17 220 строк (+1976 строк с добавлением онбординга)
- **Скомпилированный bundle (main.js):** ~500 KB
- **CSS стили:** 3 937 строк

**По компонентам:**

| Компонент | Строк кода | % | Назначение |
|-----------|-----------|---|-----------|
| ChatView.ts | 4 047 | 23% | Главный UI интерфейс чата |
| Skills module | 3 592 | 21% | 6 builtin + loader + parser + creator |
| settingsLocales.ts | 2 546 | 15% | Локализация UI настроек (8 языков) |
| settings.ts | 2 360 | 14% | Settings panel + onboarding system |
| systemPrompts.ts | 1 048 | 6% | System prompts (8 languages) |
| buttonLocales.ts | 874 | 5% | Локализация кнопок (8 языков) |
| commands.ts | 571 | 3% | Slash-commands system |
